...
Shader "UVProjection/ProjectionOnBaseMap"
{
    Properties
    {
...
[KeywordEnum(Off, On)]_DISCARDCLIPPED("Discard clipped", Float) = 0
		...
		SubShader
	{
		...
		Pass
		{
			...
			#pragma shader_feature _ _DISCARDCLIPPED_OFF _DISCARDCLIPPED_ON
...
#include "../ShaderLibrary/Projection.hlsl"
		...
		float4x4 _Projection;
			...
			float4 frag (v2f i) : SV_Target
            {
				...
				if (base.a <= 0.1)
					{
#if _DISCARDCLIPPED_ON
					discard;
#endif
					return returnColorAndNormalDot(base, 0);
				}
				...
				if (dot < _NormalClip)
				{
					...
					discard;
#endif
					return returnColorAndNormalDot(base, dot);
				}
#endif
				//return dot;
